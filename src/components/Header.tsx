...
export default function Header() {
  ...
  const NavLink = ({ href, children }: { href: string; children: React.ReactNode }) => (
    <Link href={href} className="hover:text-amber-300 transition-colors text-white/90">
      {children}
    </Link>
  );

  return (
    <header className="sticky top-0 z-40 w-full border-b border-white/10 bg-black/70">
      {/* Gold glow line */}
      <div
        aria-hidden="true"
