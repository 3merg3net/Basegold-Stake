...
const STAKING_ABI: any = [
  ...
  { type: 'function', name: 'emergencyExit', stateMutability: 'nonpayable', inputs: [{ name: 'id', type: 'uint256' }], outputs: [] },
  {
    ...
    inputs: [{ name: 'id', type: 'uint256' }, { name: 'enabled', type: 'bool' }],
    outputs: [],
  },
];
...
export default function VaultsPanel({ className }: { className?: string }) {
    ...
    return (
      <div className={className}>
        <div className="rounded-2xl border border-white/12 bg-black/50 p-5 text-white/70">
          Connect your wallet on <span className="text-[#0AA0FF] font-semibold">Base</span> to view your vaults.
        </div>
      </div>
    );
  }
  if (idList.length === 0) {
    return (
      <div className={className}>
        <div className="rounded-2xl border border-white/12 bg-black/50 p-5 text-white/70">
          No vaults yet. Stake BGLD on the{' '}
          <span className="text-amber-300 font-semibold">Stake</span> page to open your first position.
        </div>
      </div>
    );
  }
            ...
            >
              ...
              <div className="mt-3 h-2 w-full rounded-full bg-white/10 overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${progress}%`,
                    background: 'linear-gradient(90deg, rgba(212,175,55,0.25), rgba(212,175,55,0.8))',
                  }}
                />
              </div>
              ...
              <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <div className="col-span-2 sm:col-span-2 rounded-xl border border-white/10 bg-black/40 p-3">
                  <p className="text-[11px] leading-relaxed text-white/55">
                    <span className="text-white/70 font-semibold">Reminder:</span> Early exit applies
                    a decaying penalty from
                    ...
                    vested rewards. Compounding features are being wound down ahead of the V2 Gold
                    Vault — positions continue to vest normally and can be withdrawn at or before
                    maturity.
                  </p>
                </div>
              </div>

              {/* Actions (no manual Compound) */}
              {!r.closed && (
